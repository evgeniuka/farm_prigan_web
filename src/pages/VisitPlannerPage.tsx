...
const interests = ['Tasting', 'Learn about peppers', 'Compare varieties', 'Products', 'Farm highlights']
...
const comfortNeeds = ['Prefer less walking', 'Need an easier route', 'With children', 'Need more rest points']
...
function SectionCard({ letter, title, children }: { letter: string; title: string; children: ReactNode }) {
  return (
...
export function VisitPlannerPage() {
  ...
  const selectedComfort = visit.selectedComfortNeeds.length > 0
    ? visit.selectedComfortNeeds.length === 1 ? visit.selectedComfortNeeds[0] : `${visit.selectedComfortNeeds.length} selected`
    : 'No preferences'
  const selectedInterests = visit.selectedInterests.length > 0 ? visit.selectedInterests.join(', ') : 'None selected'
  ...
  return (
    <PageShell className="bg-[#fbf8f3] pb-12 pt-10 md:px-8">
      <section className="mb-8">
        ...
        <p className="mt-3 max-w-xl text-base leading-[26px] text-[#6b6359]">
          Tell us a little about your visit and we'll suggest a route that fits your time, interests, and comfort level.
        </p>
        <p className="mt-1 text-[13px] leading-5 text-[#8a7a63]">You can change everything later.</p>
      </section>

      <div className="grid gap-8 lg:grid-cols-[minmax(0,904px)_280px]">
        <div className="space-y-4">
          ...
          <SectionCard letter="C" title="What interests you most?">
            <p className="mb-3 text-[13px] leading-5 text-[#8a7a63]">Select all that apply</p>
            <div className="flex flex-wrap gap-2">
              {interests.map((interest) => {
                const active = visit.selectedInterests.includes(interest)

                return (
                  <button
                    aria-pressed={active}
                    className={`inline-flex h-[39px] items-center gap-2 rounded-full px-4 text-sm font-medium transition ${
                      active
                        ? 'border border-[var(--terracotta)] bg-[#fbe4dc] text-[var(--terracotta)]'
                        : 'border border-[#d6cdbb] bg-white text-[#2a2420] hover:bg-[#fbf8f3]'
                    }`}
                    key={interest}
          ...
          <SectionCard letter="D" title="Spice tolerance">
          ...
          <SectionCard letter="E" title="Mobility & comfort">
            <div className="grid gap-2 md:grid-cols-2">
              {comfortNeeds.map((need) => {
                const active = visit.selectedComfortNeeds.includes(need)

                return (
                  <button
                    aria-pressed={active}
                    className={`flex h-[47px] items-center gap-3 rounded-[10px] border px-4 text-left text-sm font-medium transition ${
                      active
                        ? 'border-[#6a8f4d] bg-[#eef4e5] text-[#4b5e38]'
                        : 'border-[#e8e1d3] bg-[#fbf8f3] text-[#2a2420] hover:border-[#d6cdbb]'
                    }`}
                    key={need}
                    onClick={() => toggleComfortNeed(need)}
                    type="button"
                  >
                    <span className={`flex h-[18px] w-[18px] items-center justify-center rounded-[3px] border ${active ? 'border-[#6a8f4d] bg-[#6a8f4d] text-white' : 'border-[#8a7a63]'}`}>
          ...
          <SectionCard letter="F" title="Navigation mode">
            ...
            <div className="flex gap-3">
              ...
              <div>
                ...
                <p className="text-[13px] leading-5 text-[#3e7f74]">
                  The system suggests a route based on your available time, selected interests, spice preference, and comfort needs.
                </p>
              </div>
            </div>
        ...
        <aside className="h-fit rounded-[18px] border border-[#e8e1d3] bg-white p-5 shadow-[0_10px_30px_rgba(42,36,32,0.08)] lg:sticky lg:top-24">
          ...
          <div className="mt-3">
            ...
            <SummaryRow icon={<Leaf size={14} />} label="Interests" value={selectedInterests} />
            <SummaryRow icon={<Sparkles size={14} />} label="Spice level" value={visit.selectedSpiceLevel} />
            <SummaryRow icon={<Footprints size={14} />} label="Comfort" value={selectedComfort} />
