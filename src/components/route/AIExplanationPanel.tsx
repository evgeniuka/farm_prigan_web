  ...
  return (
    <section className="quiet-panel p-5">
      <div className="mb-4 flex items-center justify-between gap-3">
        ...
        <StatusPill tone="green">{mainRoute.fitLevel}</StatusPill>
      </div>
      <div className="mb-4 flex flex-wrap gap-2">
        <StatusPill tone="cream">{visit.selectedDuration}</StatusPill>
        <StatusPill tone="cream">{visit.selectedSpiceLevel}</StatusPill>
        <StatusPill tone="cream">{visit.selectedWalkingPreference}</StatusPill>
      </div>
      <ul className="space-y-2 text-sm leading-6 text-[var(--muted)]">
        {mainRoute.recommendedBecause.map((reason) => (
